...
Codex向けの明示起動専用Pluginです。実装に入るまでは、要求理解・コードや外部情報の
...
| Main | `gpt-5.6-sol` | `xhigh` | 既存セッション。受け渡し・分割・割当・統合・通常のレビュー判定 |
...
herdr版は同一ホストのherdrが必要です。subagent版はモデル/effort指定、同一セッション
